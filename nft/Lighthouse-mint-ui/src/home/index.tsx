...
const Home = () => {
  ...
  const refreshMyMintedNfts = async () => {
    ...
    client
      ...
      .then((result) => {
        // console.log(myMintedNfts.length, result.mints.length);
        // if (myMintedNfts.length !== result.mints.length) {
        setMyMintedNfts(result.mints);
        if (myMintedNfts.length && firstLoad) {
          loadMinted();
          firstLoad = false;
        }
  ...
  const mint = async () => {
    ...
    if (
      currentPhase.max_tokens > 0 &&
      myMintedNfts.length + amount > currentPhase.max_tokens
    ) {
      toast.error(
        "You can only mint " + currentPhase.max_tokens + " tokens per wallet"
      );
    ...
    try {
      ...
      toast.dismiss(loading);
      if (e.message !== "Transaction declined") toast.error("Mint failed");
      console.log(e);
    }
  };
              ...
              {!showMintedNfts && (
                <>
                  ...
                  <C.LaunchMint>
                    ...
                    <C.Image>
                      <img src="/images/1.jpg" alt="launch" />
                    </C.Image>
                    <C.MintInfo>
                      <C.Price>
                        Price:{" "}
